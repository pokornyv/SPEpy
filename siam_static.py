...
## 9223372036854775808 == 2**63 of signed int
N = 2**NE-1
...
if GFtype in ['sc','sq']:
	...
	GFzero_F = GFlambda(En_F)
	GFzero_F = GFzero_F/IntDOS(GFzero_F,En_F)
#WriteFile(En_F,GFzero_F,GFzero_F,GFzero_F,WriteMax,WriteStep,'gf','GFzero.dat',chat)
...
Lambda = CalculateLambda(U,Bubble_F,GFzero_F,GFzero_F,En_F,T,chat,epsl)
...
if ed!=0.0:
	...
	if chat: print('# iterating the thermodynamic self-energy SigmaT, mixing: alpha = {0: .3f}:'.format(float(alpha)))
	while any([sp.fabs(nT-nTold)>epst,sp.fabs(SigmaT-SigmaT_old)>epst]):
		nTold = nT
		Lambdaold = Lambda
		SigmaT_old = SigmaT
		if GFtype in ['sc','sq']: GFtherm_F = ShiftGreensFunction(GFzero_F,En_F,-ed-SigmaT)
		else:                       GFtherm_F = GFlambda(En_F-ed-SigmaT)
		Bubble_F = TwoParticleBubble(GFtherm_F,GFtherm_F,En_F,T,'eh') # Bubble[0] is negative
		Lambda = CalculateLambda(U,Bubble_F,GFtherm_F,GFtherm_F,En_F,T,chat,epsl)
		## calculate nT ###################################
		if GFtype in ['lor','semi','gauss']:
...
if chat: print('# - ed = {0: .6f}: norm[Gint] = {1: .6f}, n[Gint] = {2: .6f}'.format(ed,float(IntDOS(GFint_F,En_F)),float(n)))
...
if chat: print('# - DOS[0] = {0: .6f}, maximum of DoS: {1: .6f} at w = {2: .6f}'.format(float(DOSF),float(DOSmax),float(wmax)))
...
if any([WriteGF,WriteVertex,WriteNpz]):
	...
	#	WriteFile(En_F,X1_F,X2_F,X3_F,WriteMax,WriteStep,header,filename,chat)
	if WriteGF:
		filename = 'gf_'+str(GFtype)+'_U'+str(U)+'eps'+str(ed)+'T'+str(T)+'.dat'
		header = '# E\t\t\tRe GF0\t\tIm GF0\t\tRe SE\t\tIm SE\t\tRe GF\t\tIm GF'
		WriteFile(En_F,GFtherm_F,SE_F,GFint_F,WriteMax,WriteStep,header,filename,chat)
	if WriteVertex:
		...
		WriteFile(En_F,Bubble_F,K_F,ChiGamma_F,WriteMax,WriteStep,header,filename,chat)
	if WriteNpz:
